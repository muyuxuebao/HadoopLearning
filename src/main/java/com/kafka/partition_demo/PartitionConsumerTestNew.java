    ...
    private void run(long maxReads, String topic, int partition_index, List<InetSocketAddress> server_address) throws Exception {
        ...
        while (maxReads > 0) {
            ...
            FetchRequest req = new FetchRequestBuilder()
                    .clientId(clientName)
                    .addFetch(metadata.leader().host(), metadata.leader().port(), readOffset, 100000) // Note: this fetchSize of 100000 might need to be increased if large batches are written to kafka
                    .build();
            ...
            if (fetchResponse.hasError()) {
                ...
                System.out.println("TTT " + fetchResponse.toString());
                short code = fetchResponse.errorCode(topic, partition_index);
                System.out.println("Error fetching data from the Broker:" + metadata.leader().host() + ":" + metadata.leader().port() + " Reason: " + code);
                if (numErrors > 5) break;
                if (code == ErrorMapping.OffsetOutOfRangeCode()) {
                    // We asked for an invalid offset. For simple case ask for the last element to reset
                    readOffset = getLastOffset(consumer, topic, partition_index, kafka.api.OffsetRequest.LatestTime(), clientName);
                    continue;
                }
                consumer.close();
                consumer = null;
//                metadata = this.findNewLeader(metadata, server_address, topic, partition_index);
                continue;
            }
            numErrors = 0;
